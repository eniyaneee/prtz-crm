$(document).ready(function () {
  $("#lug_menu_id").change(function () {
    let lug_menu_id = $(this).val();
    $.ajax({
      type: "POST",
      url: base_Url + "get-lug-submenu",
      ...
      success: function (data) {
        ...
        for (let i = 0; i < res.length; i++) {
          subAccess += `
              ...
              <option value="${res[i]["lug_submenu_id"]}">${res[i]["lug_submenu"]}</option>`;
        }
        $("#lug_submenu_id").html(subAccess);
      },
    });
  });
    ...
    if (mode == "new") {
      $.ajax({
        ...
        success: function (data) {
          ...
          $("#lug_submenu_id").html(luggageData);
        },
      });
    } else if (mode == "edit") {
      $.ajax({
        ...
        success: function (data) {
          ...
          luggageData += `<option value="${subMenuID}">${subMenu}</option>`;
          for (let i = 0; i < res.length; i++) {
            luggageData += `<option value="${res[i]["lug_submenu_id"]}">${res[i]["lug_submenu"]}</option>`;
          }
  ...
  $("#btn-submit").click(function () {
    ...
    } else if ($("#lug_submenu_id").val() === "" && mode == "new") {
  ...
  function insertData() {
    ...
    var url;
    if (mode == "new") {
      url = base_Url + "insert-lug-product";
    } else if (mode == "edit") {
      url = base_Url + "update-lug-product";
  ...
  $(document).on("click", ".btnEdit", function () {
    ...
    var lugMenu = res_DATA[index].lug_menu_id;
    subMenuID = res_DATA[index].lug_submenu_id;
    subMenu = res_DATA[index].lug_submenu;
    $("#lug_menu_id").val(lugMenu).trigger("change");
  ...
  $(document).on("click", ".BtnDelete", function () {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          type: "POST",
          url: base_Url + "delete-lug-product",
