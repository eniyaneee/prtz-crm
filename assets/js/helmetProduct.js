$(document).ready(function () {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          for (let i = 0; i < res.length; i++) {
            subMenu += `
            <option value="${res[i]["h_submenu_id"]}">${res[i]["h_submenu"]}</option>`;
          }
  ...
  $("#btn-submit").click(function () {
    $(".error").hide();
    if ($("#prod_id").val() === "" && mode == "new") {
      validateError("Please Select Menu");
    } else if ($("#h_submenu_id").val() === "" && mode == "new") {
  ...
  function insertData() {
    ...
    var url;
    if (mode == "new") {
      url = base_Url + "insert-helmet-products";
    } else if (mode == "edit") {
      url = base_Url + "update-helmet-products";
  ...
  $(document).on("click", ".btnView", function () {
    ...
    console.log(res_DATA[index].offer_price);
  ...
  $(document).on("click", ".btnEdit", function () {
    ...
    var hmenuID = res_DATA[index].h_menu_id;
    hsubMenuID = res_DATA[index].h_submenu_id;
    hsubMenu = res_DATA[index].h_submenu;
    $("#h_menu_id").val(hmenuID).trigger("change");
